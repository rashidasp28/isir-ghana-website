...
export default async function OrganizerMeetingPage({ params }: { params: { code: string; token: string } }) {
  ...
  const { data: meeting } = await supabase
    .from('meetings')
    .select('id, title, public_code, status, timezone, slot_duration_minutes, day_start_minutes, day_end_minutes, created_at')
    .eq('public_code', code)
  ...
  if (participantIds.length > 0) {
    const { data: slotData } = await supabase
      ...
      .select('meeting_date_id, start_minutes, response')
  ...
  for (const date of dates) {
    dateMap.set(date.id, date.meeting_date)
  }
  ...
  for (const slot of slots) {
    ...
    const current = scoreMap.get(key) || {
      dateId: slot.meeting_date_id,
      minute: slot.start_minutes,
      available: 0,
      maybe: 0,
      unavailable: 0,
    }
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <section className="bg-white rounded-[2rem] border border-softGray shadow-sm p-7 md:p-10 mb-7">
          ...
          <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-6">
            ...
            <Link href={`/meet/${meeting.public_code}`} className="inline-flex items-center justify-center rounded-xl bg-darkNavy text-white px-5 py-3 font-bold">
        ...
        <div className="grid md:grid-cols-3 gap-5 mb-7">
          ...
          <div className="bg-white rounded-2xl border border-softGray p-6">
            ...
            <p className="text-3xl font-bold text-darkNavy mt-1">{dates.length}</p>
          </div>
          <div className="bg-white rounded-2xl border border-softGray p-6">
            <Clock3 className="text-darkNavy mb-3" size={24} />
            <p className="text-sm text-gray-500 font-semibold">Slot size</p>
        ...
        {best && (
          <section className="rounded-[2rem] bg-lightGreen border border-green-200 p-7 md:p-9 mb-7">
            <div className="flex gap-4">
              <Sparkles className="text-primaryGreen shrink-0" />
              <div>
                <p className="text-xs uppercase tracking-[0.18em] font-bold text-primaryGreen mb-2">Best current option</p>
                <h2 className="text-2xl font-bold text-darkNavy">{formatDate(best.dateId)} at {formatTime(best.minute)}</h2>
                <p className="text-charcoal mt-2">{best.available} available · {best.maybe} maybe · {best.unavailable} unavailable</p>
              </div>
            </div>
          </section>
        )}
          ...
          {ranked.length === 0 ? (
            ...
            <div className="space-y-3">
              {ranked.map((slot, index) => (
                <div key={`${slot.dateId}:${slot.minute}`} className="grid md:grid-cols-[50px_1fr_auto] items-center gap-4 rounded-2xl border border-softGray p-4">
                  <div className="w-10 h-10 rounded-full bg-lightBlue flex items-center justify-center font-bold text-darkNavy">{index + 1}</div>
                  <div>
                    <p className="font-bold text-darkNavy">{formatDate(slot.dateId)} · {formatTime(slot.minute)}</p>
                    <p className="text-sm text-gray-500">{meeting.timezone}</p>
                  </div>
                  <div className="flex gap-2 text-xs font-bold">
                    <span className="rounded-full bg-green-100 text-green-800 px-3 py-1">{slot.available} yes</span>
                    <span className="rounded-full bg-amber-100 text-amber-800 px-3 py-1">{slot.maybe} maybe</span>
                    <span className="rounded-full bg-gray-100 text-gray-600 px-3 py-1">{slot.unavailable} no</span>
                  </div>
                </div>
              ))}
            </div>
